...
def mocked_api() -> Iterator[respx.MockRouter]:
    ...
    Pre-registers Block B `/v1/stats/runtime` and Block C `/v1/payments/networks`
    with sensible defaults so every page test doesn't have to wire them up.
    Individual tests can still override by re-registering the same route — respx
    uses the most-recently-added matcher.
    """
...
def client(mocked_api: respx.MockRouter) -> Iterator[TestClient]:
    ...
    Clears the in-process runtime cache before each test so a cached value
    from a prior test doesn't shadow a tailored mock the current test sets up.
    """
