...
# There is deliberately NO hardcoded x402 resource fallback: the old config
# mirror drifted (it advertised the legally-gated domain product and omitted
# live endpoints). The /services and /agents pages render the live
# /.well-known/x402.json, falling back to the last-good on-disk snapshot
# (see app._refresh_manifest); real-but-stale beats fabricated.
...
class Settings(BaseSettings):
    ...
    # Writable directory for the last-good x402 manifest snapshot (systemd
    # unit sets StateDirectory=hyrule-web). Snapshot writes are best-effort:
    # an unwritable dir only disables cross-restart fallback.
    state_dir: str = "/var/lib/hyrule-web"
